...
export class EventTimer extends Timer {
  ...
  broadcastState() {
    ...
    this.io.emit('numevents', this._eventlist.length);
  ...
  trigger(action, payload) {
    ...
    switch (action) {
      case 'start': {
        if (numEvents === 0 || numEvents == null) return false;
  ...
  updateEventList(eventlist) {
    ...
    const first = this._eventlist.length === 0;
  ...
  roll() {
    ...
    const numEvents = this._eventlist.length;
    if (numEvents === 0 || numEvents == null) return;
  ...
  previous() {
    ...
    if (this._eventlist.length < 1) return;
